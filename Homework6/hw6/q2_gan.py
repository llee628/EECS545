...
def bce_loss(input, target):
    ...
    x = input
    z = target
    loss = torch.clamp(x, min=0.0) - x*z + torch.log(1 + torch.exp(-torch.abs(x)))
    loss = loss.mean()
    return loss
...
def discriminator_loss(logits_real, logits_fake, dtype):
    """
    Computes the discriminator loss using the bce_loss function.
    ...
    real_loss = bce_loss(logits_real, target)
    fake_loss = bce_loss(logits_fake, 1 - target)
...
def main():
    ...
    # Train the network
    run_a_gan(D_DC, G_DC, D_DC_solver, G_DC_solver, discriminator_loss, generator_loss, loader_train, dtype, num_epochs=8)
