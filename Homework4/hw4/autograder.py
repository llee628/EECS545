...
class TestHW4(unittest.TestCase):
    ...
    def test_temporal_softmax_loss(self):
        ...
        dx_num = eval_numerical_gradient(lambda x: rnn_layers.temporal_softmax_loss(x, y, mask)[0], x, verbose=False)
        loss, dx = rnn_layers.temporal_softmax_loss(x, y, mask)
        ...
        np.testing.assert_allclose(loss, 5.4, atol=0.01)
        np.testing.assert_allclose(dx, dx_num, atol=1e-8)
