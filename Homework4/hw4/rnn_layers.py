...
def temporal_fc_backward(dout, cache):
    ...
    dw = np.dot(dout.reshape(N*T, M).T, x.reshape(N*T, D))
    dw = dw.T
...
def temporal_softmax_loss(x, y, mask):
    ...
    ###########################################################################
    # TODO: Implement the temporal softmax loss.
    ###########################################################################
    N, T, V = x.shape

    x_l = x.reshape(N*T, V)
    y_l = y.reshape(N*T)
    mask_l = mask.reshape(N*T)
    ...
    #breakpoint()
    loss = 0
    indicator = np.zeros_like(x_l)

    for i in range(N*T):
        indicator[i, y_l[i]] = 1
        loss += np.log(p[i, y_l[i]])*mask_l[i]
        #breakpoint()

    loss = -loss/N
    dx = (p - indicator)/N
    dx = dx*mask_l.reshape(mask_l.shape[0],1)
    #breakpoint()
    dx = dx.reshape(N,T,V)
